...
#### 2026-03-13 - V1.5.0 Token刷新机制
**新增功能：**
- ✅ **双Token机制**: 访问令牌(7天) + 刷新令牌(30天)
- ✅ **自动刷新**: 每30分钟自动检查并刷新Token
- ✅ **智能检测**: Token即将过期(1小时内)自动刷新
- ✅ **页面可见性**: 切换回页面时自动检查Token
- ✅ **自动重试**: API请求401时自动刷新并重试

**技术实现：**
- `POST /api/auth/refresh` - 刷新Token API
- `useAuth()` Hook - 自动刷新逻辑
- `authFetch()` - 自动处理401的fetch封装
- 刷新失败自动跳转登录页

**GitHub提交**: [9122cdf](https://github.com/cooleye/private-traffic-system/commit/9122cdf)

---

#### 2026-03-13 - V1.6.0 地域分布统计功能
**新增功能：**
- ✅ **IP转地域**: 根据IP自动识别国家、省份、城市
- ✅ **省份分布**: 统计页面显示省份分布 TOP15 横向柱状图
- ✅ **城市分布**: 统计页面显示城市分布 TOP15 横向柱状图
- ✅ **自动记录**: 访问时自动记录地域信息到数据库

**技术实现：**
- 创建 `lib/geo.ts` IP转地域服务
- 支持中国主要省份和城市的IP映射
- 本地IP和未知IP自动过滤
- 数据库已支持 `country`, `province`, `city` 字段

**GitHub提交**: [75a00a2](https://github.com/cooleye/private-traffic-system/commit/75a00a2)

---

#### 2026-03-13 - V1.7.0 转化统计功能
**新增功能：**
- ✅ **转化漏斗**: 追踪用户从点击到添加成功的完整链路
- ✅ **转化统计**: 统计页面显示转化率概览
- ✅ **漏斗可视化**: 三阶段漏斗图（点击→复制→添加成功）
- ✅ **会话追踪**: 使用 sessionId 关联同一用户的多个行为

**技术实现：**
- 创建 `ConversionLog` 模型记录转化事件
- `POST /api/conversion/track` - 转化追踪 API
- 支持 VIEW、COPY、ADD_SUCCESS 三种行为
- 自动计算各阶段转化率

**GitHub提交**: [ca7acb1](https://github.com/cooleye/private-traffic-system/commit/ca7acb1)

---

#### 2026-03-13 - V1.8.0 数据导出功能
**新增功能：**
- ✅ **数据导出**: Dashboard 支持导出多种数据类型
- ✅ **导出类型**: 短链接、访问记录、转化记录、统计数据
- ✅ **CSV格式**: 导出为CSV文件，兼容Excel
- ✅ **时间筛选**: 支持按时间范围导出（默认30天）

**技术实现：**
- `GET /api/export?type=&days=` - 数据导出API
- 自动生成带日期的文件名
- 自动处理特殊字符（逗号、引号等）

**GitHub提交**: [9b0b1f5](https://github.com/cooleye/private-traffic-system/commit/9b0b1f5)

---

#### 2026-03-13 - V1.9.0 短链接分页功能
**新增功能：**
- ✅ **分页查询**: 短链接列表支持分页显示
- ✅ **分页组件**: Dashboard 添加分页导航
- ✅ **页码跳转**: 支持上一页/下一页和页码跳转
- ✅ **分页信息**: 显示总条数和当前页码

**技术实现：**
- API 支持 `page` 和 `limit` 参数
- 每页默认显示10条数据
- 搜索筛选时自动回到第一页

**GitHub提交**: [89d87b7](https://github.com/cooleye/private-traffic-system/commit/89d87b7)
